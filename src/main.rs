...
mod tests {
    ...
    fn test_new_project_creates_dir_and_readme() {
        ...
        let code = run_with_vault(&vault, false);
        assert_eq!(code, 0);
        assert!(vault.root.join("Inbox/Processed/new-proj.md").exists());
        ...
        let envelope_file = fs::read_dir(vault.root.join("Outbox")).unwrap()
            .filter_map(|e| e.ok())
            .find(|e| e.path().extension().is_some_and(|ext| ext == "json")).unwrap();
        let envelope: serde_json::Value = serde_json::from_str(
            &fs::read_to_string(envelope_file.path()).unwrap()).unwrap();
    ...
    fn test_research_handler_produces_output() {
        ...
        let code = run_with_vault(&vault, false);
        assert_eq!(code, 0);
        let research: Vec<_> = fs::read_dir(vault.root.join("Outbox")).unwrap()
            .filter_map(|e| e.ok())
            .filter(|e| e.path().to_string_lossy().contains(".research.md")).collect();
    ...
    fn test_question_handler_produces_answer() {
        ...
        let code = run_with_vault(&vault, false);
        assert_eq!(code, 0);
        assert!(vault.root.join("Inbox/Processed/q.md").exists());
        let answers: Vec<_> = fs::read_dir(vault.root.join("Outbox")).unwrap()
            .filter_map(|e| e.ok())
            .filter(|e| e.path().to_string_lossy().contains(".research.md")).collect();
    ...
    fn test_tag_routing_existing_project() {
        ...
        let code = run_with_vault(&vault, false);
        assert_eq!(code, 0);
        assert!(vault.root.join("Inbox/Processed/tag.md").exists());
        let envelope_file = fs::read_dir(vault.root.join("Outbox")).unwrap()
            .filter_map(|e| e.ok())
            .find(|e| e.path().extension().is_some_and(|ext| ext == "json")).unwrap();
        let envelope: serde_json::Value = serde_json::from_str(
    ...
    fn test_openclaw_invalid_json_unenriched() {
        ...
        unsafe {
            ...
            std::env::set_var("DIGEST_POLICY",
                PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("config/policy.json").to_str().unwrap());
        }
        vault.place_inbox("invalid.md", "Some random note.");
        let code = run(Some(vault.root.to_string_lossy().to_string()), false, 10, true);
        assert_eq!(code, 0);
        assert!(vault.root.join("Inbox/Processed/invalid.md").exists());
        let envelope_file = fs::read_dir(vault.root.join("Outbox")).unwrap()
            .filter_map(|e| e.ok())
            .find(|e| e.path().extension().is_some_and(|ext| ext == "json")).unwrap();
        let envelope: serde_json::Value = serde_json::from_str(
    ...
    fn test_log_entry_format() {
        ...
        let code = run_with_vault(&vault, false);
        assert_eq!(code, 0);
        let logs: Vec<_> = fs::read_dir(vault.root.join("Logs")).unwrap()
    ...
    fn test_policy_cheap_model_for_classification() {
        ...
        vault.place_inbox("policy1.md", "Pick up groceries.");
        let code = run(Some(vault.root.to_string_lossy().to_string()), false, 10, true);
        assert_eq!(code, 0);
    ...
    fn test_policy_mid_model_for_enrichment() {
        ...
        vault.place_inbox("policy2.md", "Some task for enrichment.");
        let code = run(Some(vault.root.to_string_lossy().to_string()), false, 10, true);
        assert_eq!(code, 0);
        let log = fs::read_to_string(&model_log).unwrap();
    ...
    fn test_policy_missing_file_no_model_args() {
        ...
        let code = run(Some(vault.root.to_string_lossy().to_string()), false, 10, true);
        assert_eq!(code, 0);
        assert!(vault.root.join("Inbox/Processed/nopol.md").exists());
        if model_log.exists() {
            let log = fs::read_to_string(&model_log).unwrap();
